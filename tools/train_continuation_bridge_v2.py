"""Train Continuation Bridge V2 using a multi-stage curriculum (short, medium, full)."""
...
def save_checkpoint(engine, step):
    ckpt = {
        ...
        'phase': f'continuation_bridge_v2_step_{step}'
    }
...
def main() -> None:
    ...
    print("[TRAIN] Loading engine v2 for Continuation Bridge V2...", flush=True)
    ...
    print(f"[TRAIN] Prepared {len(dataset)} training template references for multi-stage curriculum", flush=True)
    ...
    elapsed = time.time() - t0
    print(f"\n[TRAIN DONE] Completed V2 curriculum sweep in {elapsed:.1f}s ({elapsed/60:.1f}m)", flush=True)
