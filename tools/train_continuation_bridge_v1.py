"""Train Continuation Bridge V1 to optimize generation after retrieved anchors."""
...
def save_checkpoint(engine, step):
    ckpt = {
        ...
        'phase': f'continuation_bridge_v1_step_{step}'
    }
...
def main() -> None:
    ...
    print("[TRAIN] Loading engine v2 for Continuation Bridge V1...", flush=True)
    ...
    # Construct the training dataset from the 75 prompts and their 5 variants
    # Each sample contains: prefix_tokens, continuation_tokens, domain, intent
    dataset = []
    ...
    for mode, pairs in PAIRS.items():
        for prompt, expected in pairs:
            ...
            # Map domain routing identically to SpeechControllerV1 inference
            domain = "conversation"
            if intent == "identity":
                domain = "identity"
            ...
            # Get retrieved 5-token anchor prefix
            anchor_tokens = engine.enc.encode(expected)[:5]
            retrieved_start = engine.enc.decode(anchor_tokens)
            
            # Remaining continuation target
            continuation_tokens = engine.enc.encode(expected)[5:] + [engine.enc.eot_token]
            ...
            for var in variants:
                ...
                # Construct exact prefix used at inference
    ...
    print(f"[TRAIN] Prepared {len(dataset)} training samples representing all variants", flush=True)
    ...
    for step in range(1, STEPS + 1):
        ...
        input_tokens = prefix_tokens + continuation_tokens
        labels = [-100] * len(prefix_tokens) + continuation_tokens
        ...
        # Calculate loss (excluding prefix via cross-entropy ignore_index=-100)
        ...
        if step % 100 == 0 or step == 1:
            ...
            print(f"  Step {step:4d}/{STEPS} | Loss: {loss_val:.4f} | LR: {current_lr:.2e} | Domain: {domain:<12} | {elapsed:.0f}s", flush=True)
    ...
    elapsed = time.time() - t0
    print(f"\n[TRAIN DONE] Completed {STEPS} steps in {elapsed:.1f}s ({elapsed/60:.1f}m)", flush=True)
